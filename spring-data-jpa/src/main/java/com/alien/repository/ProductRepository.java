...
import com.alien.entiry.Product;
...
public interface ProductRepository extends JpaRepository<Product, Integer> {
	...
	// @Query(value = "SELECT * FROM PRODUCT_TBL WHERE price = ?1",nativeQuery =
	// true)
	@Query("from Product p where p.price= ?1 ")
	// position based parameter
	// @Query("from PRODUCT_TABLE p where p.price= :price") //named parameter base
	// index
